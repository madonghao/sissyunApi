...
!> 注意不是货号是ID,ID需要从获取缓存数据里拿到

参数 | 是否必须 | 描述 | 默认值
:-:|:-:|:-:|:-:
`data` | 是 | 需要添加的数据的ID可以是数组也可是字符串 | ''
`cover` | 否 | 1代表覆盖现有类别和名称 0代表不覆盖现有类别和名称 | 1
`type` | 否 | 1代表会员价 0代表零售价 | 1

**接口地址：** `/add/list`

**调用例子：** `/add/list?data=653467,653508&cover=1&type=1`
